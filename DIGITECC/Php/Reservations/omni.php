...
session_start();
    ...
    $motivation = $_POST['Fonction'];
    if (isset($_POST['check'])  ) {

        $sub = "select * from reservation_omnisport";
        $result = mysqli_query($con,$sub);
    while ($row = mysqli_fetch_assoc($result)) {
        if ($motivation == $row['motivation']) {
            $daatt = $row['date_res'];
            echo ("<style>[id^='$daatt']{background-color: red;}}</style>");
        }
    }
    $sub = "select * from reservation_omnisport where email='$user_daa[email]'";
    $result = mysqli_query($con,$sub);
while ($row = mysqli_fetch_assoc($result)) {
    if ($motivation == $row['motivation']) {
        $daatt = $row['date_res'];
        ...
        $quee = "select count(*) as total from reservation_omnisport where email='$user_daa[email]' ;";
        $result = mysqli_query($con, $quee);
        $number = mysqli_fetch_assoc($result);
        $queee = "select count(*) as em from reservation_omnisport where date_res='$datte' and motivation='$motivation';";
        ...
        if ($nuu['em'] == 0) {
            ...
            if ($number['total'] >= 1) {
                ...
                echo "<center><br><label style='color:white;'>Vous avez atteint le nombre maximal de réservations";
            } else {
                // $date = $the[$x];
                $query = "insert into reservation_omnisport (email,nom,prenom,num,date_res,motivation) values ('$user_daa[email]','$user_daa[nom]','$user_daa[prenom]','$user_daa[num]','$datte','$motivation');";
                ...
                for ($x = 0; $x <= count($the); $x++) {
                    ...
                    if ( $a == $datte) {
                        echo "<center><br><label style='color:white;'>Vous avez réservé l'omnisport pour la motivation de $motivation pour la date $tran[$x]";
                    }
                }
            }
                
        } else {
            $qqq = "select * from reservation_omnisport where date_res='$datte' and motivation='$motivation' limit 1 ;";
            ...
            $word.=$nuua['num'];
            echo "<center><br><label style='color:white;'>$word</label></center>";
        }
    } else if (isset($_POST['cancel']) && !empty($_POST['button'])) {
        $datte = $_POST['button'];

        $qeey = "select * from reservation_omnisport where date_res='$datte' and motivation='$motivation';";
        $res = mysqli_query($con, $qeey);
        $datta = mysqli_fetch_assoc($res);
        if (!empty($datta)) {
            if ($datta['email'] == $user_daa['email']) {
                $cmd = "delete from reservation_omnisport where date_res = '$datte' and motivation = '$motivation';";
                mysqli_query($con, $cmd);
                echo "<center><br><label style='color:white;'>Vous avez annuler votre résérvation</label>";
            }
            else if ( $user_daa['fct']=="Bureaux" &&  $user_daa['fct']=="Personnel" && $user_daa['fct']=="Association") {
                $cmd = "delete from reservation_omnisport where date_res = '$datte' and motivation = '$motivation';";
                mysqli_query($con, $cmd);
                echo "<center><br><label style='color:black;'>Vous avez annuler cette résérvation</label>";
            } else {
                echo "<center><br><label style='color:white;'>You don't have the permission to cancel this reservation</label>";
            }
        }
        else{
...
<head>
    ...
    <link rel="stylesheet" href="../../Styles/Reservations/omni.css">
    ...
    <div class="navigation">
      ...
      <div class="logout">LOGOUT</div>
        ...
        <select id="Fonction" class="Fonction" name="Fonction">
            ...
            <option value="badminthon">Badminthon</option>
            <option value="tennis">Tennis</option>
            </select><br><br>
        <div style="color:white;">Choissir le créneau qui vous intéresse et cocher sur Reserver  :</div><br>
        ...
        <script type="text/javascript" src="../../Script/Reservations/omni.js"></script>
...
<?php 
        ...
        for ($i = 0; $i < count($semaines); $i++) {
            ...
            if ($today==$daysOfWeek[$i]){
                echo "Done";
                $e=($i-1)%7;
                $queryy="delete from reservation_omnisport where date_res like '%$semaines[$e]';";
