...
session_start();
    ...
    $salle = $_POST['Fonction'];
    if (isset($_POST['check'])  ) {

        $sub = "select * from reservation";
    ...
    $sub = "select * from reservation where email='$user_daa[email]'";
    $result = mysqli_query($con,$sub);
while ($row = mysqli_fetch_assoc($result)) {
    if ($salle == $row['salle']) {
        $daatt = $row['date_res'];
        ...
        $quee = "select count(*) as total from reservation where email='$user_daa[email]' ;";
        $result = mysqli_query($con, $quee);
        $number = mysqli_fetch_assoc($result);
        $queee = "select count(*) as em from reservation where date_res='$datte' and salle='$salle';";
        ...
        if ($nuu['em'] == 0) {
            ...
            if ($number['total'] >= 3) {
                ...
                // $date = $the[$x];
                $query = "insert into reservation (email,nom,prenom,num,date_res,salle) values ('$user_daa[email]','$user_daa[nom]','$user_daa[prenom]','$user_daa[num]','$datte','$salle');";
                ...
                for ($x = 0; $x <= count($the); $x++) {
                    ...
                    if ( $a == $datte) {
                        echo "<center><br><label style='color:white;'>Vous avez réservé la salle $salle pour la date $tran[$x]";
                    }
                }
            }
                
        } else {
            $qqq = "select * from reservation where date_res='$datte' and salle='$salle' limit 1 ;";
        ...
        $qeey = "select * from reservation where date_res='$datte' and salle='$salle';";
        $res = mysqli_query($con, $qeey);
        $datta = mysqli_fetch_assoc($res);
        if (!empty($datta)) {
            if ($datta['email'] == $user_daa['email']) {
                $cmd = "delete from reservation where date_res = '$datte' and salle = '$salle';";
...
<head>
    ...
    <link rel="stylesheet" href="../../Styles/Reservations/salles.css">
...
<body>
    ...
    <div class="navigation">
      ...
      <div class="logout">LOGOUT</div>
        ...
        <center>
        <form method="post"><br>
        <div style="color:white;">Choissir la salle qui vous intéresse :</div>
        ...
        <script type="text/javascript" src="../../Script/Reservations/salles.js"></script>
...
<?php 
        ...
        for ($i = 0; $i < count($semaines); $i++) {
            ...
            if ($today==$daysOfWeek[$i]){
                echo "Done";
                $e=($i-1)%7;
                $queryy="delete from reservation where date_res like '%$semaines[$e]';";
