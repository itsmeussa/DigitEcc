...
session_start();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['check'])  ) {
            ...
            $sub = "select * from reservation_gym";
            $result = mysqli_query($con,$sub);
            while ($row = mysqli_fetch_assoc($result)) {
                $daatt = $row['date_res'];
                echo ("<style>[id^='$daatt']{background-color: red;}}</style>");
            }
            $sub = "select * from reservation_gym where email='$user_daa[email]'";
    $result = mysqli_query($con,$sub);
while ($row = mysqli_fetch_assoc($result)) {
    $daatt = $row['date_res'];
    ...
    $quee = "select count(*) as total from reservation_gym where email='$user_daa[email]' ;";
    $result = mysqli_query($con, $quee);
    $number = mysqli_fetch_assoc($result);
    $queee = "select count(*) as em from reservation_gym where date_res='$datte';";
        ...
        if ($nuu['em'] == 0) {
            ...
            if ($number['total'] >= 10) {
                ...
                echo "<center><br><label style='color:black;'>Vous avez atteint le nombre maximal de réservations";
            } else {
                // $date = $the[$x];
                $query = "insert into reservation_gym (email,nom,prenom,num,date_res) values ('$user_daa[email]','$user_daa[nom]','$user_daa[prenom]','$user_daa[num]','$datte');";
                ...
                for ($x = 0; $x <= count($the); $x++) {
                    ...
                    if ( $a == $datte) {
                        echo "<center><br><label style='color:black;'>Vous avez réservé la salle de gymnase pour la date $tran[$x]";
                    }
                }
            }
            
        } else {
            $qqq = "select count(*) as tot from reservation_gym where date_res='$datte';";
            $resultaa = mysqli_query($con, $qqq);
            $nuua = mysqli_fetch_assoc($resultaa);
            $qeey = "select * from reservation_gym where date_res='$datte';";
            $res = mysqli_query($con, $qeey);
            $datta = mysqli_fetch_assoc($res);
            if ($nuua['tot']<20 && $datta['email'] != $user_daa['email'] && !empty($datta) ){
                $nn=$nuua['tot'];
                echo ("<center><br><label style='color:black;'>Vous avez réservez la salle de gymnase avec $nn de personnes ");
                $query = "insert into reservation_gym (email,nom,prenom,num,date_res) values ('$user_daa[email]','$user_daa[nom]','$user_daa[prenom]','$user_daa[num]','$datte');";
                mysqli_query($con, $query);
            }
            else if ($nuua['tot']>=20) {
                echo "<center><br><label style='color:black;'>le nombre maximum de réservation est atteint";
            }
            else{
                echo "<center><br><label style='color:black;'>Vous avez déja reservez ce créneau";
            }
        }
        ...
        if (!empty($datta)) {
            if ($datta['email'] == $user_daa['email']) {
                $cmd = "delete from reservation_gym where date_res = '$datte';";
                mysqli_query($con, $cmd);
                echo "<center><br><label style='color:black;'>Vous avez annuler votre résérvation</label>";
            }
                ...
                echo "<center><br><label style='color:black;'>You don't have the permission to cancel this reservation</label>";
            }
        }
        else{
...
<html lang="en">
    ...
    <link rel="stylesheet" href="../../Styles/Reservations/gym.css">
...
<body>
    ...
    <div class="navigation">
      ...
      <div class="logout">LOGOUT</div>
        ...
        <script type="text/javascript" src="../../Script/Reservations/gym.js"></script>
        ...
        for ($i = 0; $i < count($semaines); $i++) {
            ...
            if ($today==$daysOfWeek[$i]){
                echo "Done";
                $e=($i-1)%7;
                $queryy="delete from reservation_gym where date_res like '%$semaines[$e]';";
